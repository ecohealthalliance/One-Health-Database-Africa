...
knitr::opts_chunk$set(echo = FALSE, messages = F, warning = F, comment = "")
...
workforce <- filtered_data %>%
  ...
  rename("Value" = "value", "Value per 1000 people" = per_1000_capita, 
         "Value per 1000 sq.km" = per_1000_km, Indicator = name, Year = year) %>%
...
amr <- filtered_data %>%
  ...
  rename(Indicator = indicator, Value = value)
...
jee <- filtered_data %>%
  ...
  mutate(indicator = fct_recode(indicator, "D.3.1 System for efficient reporting to FAO, OIE and WHO" = "D.3.1 System for e cient reporting to FAO, OIE and WHO" ))
 

jee_yr <- jee %>%
  dplyr::select(year) %>%
  droplevels() 
jee_year <- unique(as.character(jee_yr$year))
...
if(all(is.na(jee$value))){
  print("The country had not completed a JEE as of 2021")
} else { 
  knitr::kable(jee_table)}
...
These data were reported in 2020


```{r spar}

spar <- filtered_data %>%
  filter(indicator %in% c("C.3.1", "C.4.1", "C.5.1", "C.5.3", "C.6.1", "C.6.2", "C.11.1", "C.11.2")) %>%
  select(c(indicator, value)) %>%
  mutate(indicator = fct_recode(indicator, "C.3.1 Collaborative effort on activities to address zoonoses" = "C.3.1",
...
```{r disease numbers, message=FALSE}
...
disease_n <- left_join(disease_number, pops_areas) %>%
  ...
  mutate(Indicator = fct_recode(Indicator, "Rabies - number of deaths" = "rabies deaths", 
                                "Malaria - estimated number of cases" = "Estimated number of malaria cases")) %>%
  mutate(Indicator = as.character(Indicator)) %>%
  dplyr::arrange(Indicator)
...
env <-  filtered_data %>%
  ...
  mutate(units = fct_recode(units, "Percent" = "percent")) %>%
...
demo_factors <- c("cfe allocation" ,  
                  ...
                  "Population, total" ,
                  "Stability", "Protein", "Percent animal protein", "AnimalProtein" )
...
demo <- filtered_data %>%
  ...
  mutate(value = as.numeric(as.character(value))) %>%
  mutate(value = round(value, 2)) %>%
  mutate(indicator = fct_recode(indicator, "Political stability and absence of violence/terrorism" = "Stability", 
                                ...
                                "GNI per capita, Atlas method" = 
                                  "GNI per capita, Atlas method (current US$)",
                                "Protein consumption - total" = "Protein",
                                "Protein consumption - animal" = "AnimalProtein", 
                                "Protein consumption - percentage animal protein" = "Percent animal protein")) %>%
  mutate(units = fct_recode(units, "Percent" = "percent",
                            "Aggregate indicator in units of a normal standard distribution (~ - 2.5-2.5)" =  "Aggregate indicator in units of a normal standard distribution (~ - 2.5-2.5) - Political Stability and Absence of Violence/Terrorism")) %>% 
  rename(Indicator = indicator, Year = year, Value = value, Units = units) %>%
   mutate(Indicator = as.character(Indicator)) %>%
  dplyr::arrange(., Indicator)
...
#### **Rift Valley Fever Indicators**

For all of the variables reported except "Number of publications reporting RVF" the outputs are coded as:
1 = Yes 
0 = No 
...
RVF_factors <- c("RVFany", "RVFAnimalReport", "RVFHumanReport", "RVFAnimalandHuman",
                 "Publicationspercountry")

# Not filtering by year here. 
rvf <- filtered_data %>%
  filter(indicator %in% RVF_factors) %>%
  dplyr::select(indicator, value) %>%
  mutate(indicator = fct_recode(indicator, "RVF - any reports of RVF within the country" = "RVFany", 
                                "RVF - any reports of RVF in animals within the country" =
                                  "RVFAnimalReport",
                                "RVF - any reports of RVF in people within the country" = 
                                  "RVFHumanReport",
                                "RVF - any reports of RVF in human or animals within the country" = 
                                  "RVFAnimalandHuman",
                                "Number of publications reporting RVF" = 
                                  "Publicationspercountry")) %>%
  rename(Indicator = indicator,  Value = value) %>%
   mutate(Indicator = as.character(Indicator)) %>%
  dplyr::arrange(., Indicator)
