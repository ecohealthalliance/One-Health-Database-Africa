...
# source("top_script_api.R")

#full_data_factor_api <- read.csv("Output/full_data_factor_api.csv")
#full_data_number_api <- read.csv("Output/full_data_number_api.csv", stringsAsFactors = T)
#levels(full_data_number_api$indicator)

full_data <- read.csv("Output/full_data_combined_api.csv", stringsAsFactors = T)

# select the years and data that you want.

# filtered_data_factor <- full_data_factor_api %>%
#  filter(country %in% select_country)  # not selecting year as limited years available for many of these factors. 

# filtered_data_number <- full_data_number_api %>%
...
disease <- filtered_data %>%
  filter(indicator %in% c("promed events 2015-2021", "Presence of porcine cysticercosis", "Estimated number of malaria cases",
                          "water_national_at_least_basic", "water_rural_at_least_basic", "rabies deaths", 
                          "Robust national rabies surveillance",  "Robust national rabies control program implemented",
                          "Vaccine Availability", "RIG Availability", "Yellow fever - number of reported cases"))
...
disease_factor$indicator <- fct_recode(disease_factor$indicator, "Rabies Vaccine Availability" = "Vaccine Availability")
...
disease_perc <- disease %>%
  filter(type == "integer") %>%
  filter(!indicator == "promed events 2015-2021") %>%
  filter(year %in% select_year) %>%
  filter(units == "percent") %>%
  dplyr::select(indicator, year, value) %>%
  mutate("Value per 1000 people" = NA) %>%
  mutate("Value per 1000 sq.km" = NA) %>%
  rename("Indicator" = indicator, "Value" = value, Year = year) %>%
  mutate(Indicator = fct_recode(Indicator, "Percentage of population with at least basic water (national)" 
                                = "water_national_at_least_basic", 
                                "Percentage of population with at least basic water (rural)" = "water_rural_at_least_basic"))
...
## Demography and population wealth
